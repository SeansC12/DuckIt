...
export default function home() {
  ...
  const stopRecording = useCallback(async () => {
    ...
    if (selectError) {
      alert("Something went wrong with getting topics");
      return;
    }
    const topicCount = topics.length;
    // console.log(topicCount);
    const topicTitle = `Topic ${topicCount + 1}`;
    const { data: topic, error: topicInsertError } = await supabase.from("topics")
    .insert({ topic_title: topicTitle })
    .select()
    .single();

    if (topicInsertError || !topic) {
      alert("Something went wrong with creating topic");
      return;
    }
    router.refresh();

    // theres something wrong with creating sessions.
    ...
    console.log("works, still need router push not done yet");
    router.push(`/t/${topic.id}/sessions`);
  ...
  return (
    <div className="px-20 py-10 space-y-8">
      ...
      <div className="flex space-y-8 items-center flex-col w-full max-w-[1600px]">
        ...
        <div
          className={`w-70 h-70 rounded-full bg-background border-4 transition-colors duration-200 ${recordingState.isTalking
            ? "border-green-600"
            : listening
              ? "border-yellow-600"
              : "border-red-500"
